...
const Movies = ({ isUnsolved, isStudentView = false, isPlacement = false, isTraining = false } = {}) => {
    ...
    const fetchAPI = () => {
        fetch(
            isStudentView
                ? '/mypost'
                : `/allpost?${isUnsolved ? 'status=PENDING&' : ''}${isPlacement ? 'type=PLACEMENT&' : ''}${
                    ...
                    {data && data.length
                        ? data.map(
                              (
                                  ...
                                  <tr key={id} style={{ cursor: 'pointer' }} onClick={() => handleMovieClick(id)}>
                                      <td>{index + 1}</td>
                                      <td>{userName}</td>
                                      <td>{subject}</td>
                                      ...
                                      {!isStudentView ? (
                                          <td
                                              ...
                                              <button
                                                  ...
                                                  onClick={() => {
                                                      const updatedStatus = statusUp[id];
                                                      if (updatedStatus) {
                                                          fetch(`/query/${id}`, {
                                                              method: 'put',
                                                              headers: {
